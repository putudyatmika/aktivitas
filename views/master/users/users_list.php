                ...
                <div class="ibox float-e-margins">
                    ...
                    if ($r_user["error"]==false) {
                        ...
                        for ($i=1;$i<=$total_users;$i++) {
                            if ($r_user["item"][$i]["tgl_lastlogin"]=='0000-00-00 00:00:00') {
                            ...
                            echo '
                                <tr>
                                    ...
                                    <td>'.$lvl_user[$r_user["item"][$i]["level"]].'</td>
                                    <td>'.$r_user["item"][$i]["unit_nama"].'</td>
                                    <td>'.$status_umum[$r_user["item"][$i]["aktif"]].'</td>
                                    <td>'.$lastlog_user.'</td>
                                    <td>'.$r_user["item"][$i]["ip_lastlogin"].'</td>
                                    <td>'.$r_user["item"][$i]["nohp"].'</td>
                                    <td><div class="tooltip-bps"><a href="'.$url.'/'.$page.'/'.$act.'/view/'.$r_user["item"][$i]["id"].'" class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="top" title="View User '.$r_user["item"][$i]["nama"].'"><i class="fa fa-search" aria-hidden="true"></i></a> <a href="'.$url.'/'.$page.'/'.$act.'/edit/'.$r_user["item"][$i]["id"].'" class="btn btn-warning btn-xs" data-toggle="tooltip" data-placement="top" title="Edit User '.$r_user["item"][$i]["nama"].'"><i class="fa fa-pencil-square" aria-hidden="true"></i></a> <a href="'.$url.'/'.$page.'/'.$act.'/delete/'.$r_user["item"][$i]["id"].'" data-confirm="Apakah data ('.$r_user["item"][$i]["username"].') '.$r_user["item"][$i]["nama"].' ini akan di hapus?" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Hapus User '.$r_user["item"][$i]["nama"].'"><i class="fa fa-trash-o" aria-hidden="true"></i></a></div></td>
                                </tr>
                            ';
                        }
                    }
