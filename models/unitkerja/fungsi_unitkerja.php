...
function list_unitkerja($unit_kode,$detil=false,$jenis=false,$eselon3=false,$kode_eselon) {
	...
	$conn_unit = $db_unit -> connect();
	if ($detil==true) {
		$sql_unit = $conn_unit -> query("select * from unitkerja left join (select kode as parent_kode, nama as parent_nama from unitkerja where jenis='1' and SUBSTRING(kode,5,1)=0 order by kode asc) as unitparent on unitkerja.parent=unitparent.parent_kode where kode='$unit_kode'");
	}
	else {
		if ($jenis==false) {
			if ($eselon3==false) {
				if ($kode_eselon>0) {
					$sql_unit = $conn_unit -> query("select * from unitkerja left join (select kode as parent_kode, nama as parent_nama from unitkerja where jenis='1' and SUBSTRING(kode,5,1)=0 order by kode asc) as unitparent on unitkerja.parent=unitparent.parent_kode where jenis='1' and eselon='$kode_eselon' order by kode asc");
				}
				else {
					$sql_unit = $conn_unit -> query("select * from unitkerja left join (select kode as parent_kode, nama as parent_nama from unitkerja where jenis='1' and SUBSTRING(kode,5,1)=0 order by kode asc) as unitparent on unitkerja.parent=unitparent.parent_kode where jenis='1' order by kode asc");
				}
				
			}
			else {
				$sql_unit = $conn_unit -> query("select * from unitkerja left join (select kode as parent_kode, nama as parent_nama from unitkerja where jenis='1' and SUBSTRING(jenis,5,1)=0 order by kode asc) as unitparent on unitkerja.parent=unitparent.parent_kode where jenis='1' and eselon='3' and SUBSTRING(kode,5,1)=0 order by kode asc");	
			}
		}
		else {
	...
	if ($cek_unit>0) {
		...
		while ($r=$sql_unit->fetch_object()) {
			$unitkerja_list["item"][$i]=array(
				"unit_kode"=>$r->kode,
				"unit_nama"=>$r->nama,
				"unit_parent"=>$r->parent,
				"parent_nama"=>$r->parent_nama,
				"unit_jenis"=>$r->jenis,
				"unit_dibuat_waktu"=>$r->tgl_add,
				"unit_dibuat_oleh"=>$r->add_oleh,
				"unit_diupdate_waktu"=>$r->tgl_update,
				"unit_diupdate_oleh"=>$r->update_oleh,
				"unit_eselon"=>$r->eselon
			);
			$i++;
		}
	}
